...
// Función auxiliar para devolver el mínimo de 3 posibles carvings
// Devuelve el primer nodo de ese camino con la energia total
...
std::pair<int, double> PD(const std::vector<std::vector<double>>& energia, int i, int j, int n, int m, std::vector<std::vector<std::pair<int, double>>> & memo) {
    if(j >= 0 && j < m) {
        // Si ya calcule la pos, lo devuelvo
        ...
        // CASO BASE: llegué a la última fila
            ...
            std::pair<int, double> abajo = PD(energia, i+1, j, n, m, memo); // BAJO VERTICAL
            std::pair<int, double> izq = PD(energia, i+1, j-1, n, m, memo); // BAJO A LA IZQ
            std::pair<int, double> der = PD(energia, i+1, j+1, n, m, memo); // BAJO A LA DER
...
std::vector<int> encontrarSeamPD(const std::vector<std::vector<double>>& energia) {
    ...
    // Popular memo con elementos vacios. {-1 y NaN}
    for (int i = 0; i < n; i++){
    ...
    // El primer nodo del camino con menor energia y la energia de ese camino
    int best = -1; 
    double best_energia = INFINITY;

    for(int i = 0; i < m; i++){
        std::pair<int, double> stream = PD(energia, 0, i, n, m, memo);
        if(stream.second < best_energia){ // Si se encuentra un camino mejor, guardarlo en best
            best = stream.first;
